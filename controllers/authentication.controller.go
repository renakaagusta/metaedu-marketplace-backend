...
func (ac *AuthenticationController) GetUserNonce(ctx *gin.Context) {
	...
	if (existingUser == models.User{}) {
		ctx.JSON(http.StatusCreated, gin.H{"status": "failed", "error": "User doesn't exist"})
		return
	}
...
func (ac *AuthenticationController) SignIn(ctx *gin.Context) {
	...
	err = ac.repository.UpdateUser(user)
	...
	signedToken, err := ac.jwtProvider.CreateJWT(user.Address)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"status": "failed", "error": "Failed to create token access"})
		return
	}

	ctx.JSON(http.StatusCreated, gin.H{"status": "success", "data": gin.H{"accessToken": signedToken}})
}
